...
//    }
    ...
    public List<ResAsDataJoinBusinessData> selectASdataJoinBusinessDataForExcelDownload(SearchDto params){

        System.out.println(params.toString());
        // 게시글 수 & 총 발전 용량 조회
        int count = businessDataMapper.countForAsDataJoinBusinessData(params);
        // 등록된 게시글이 없는 경우, 로직 종료
        if (count < 1) {
            return null;
        }
        ...
        // 게시글 리스트 조회
        List<ResAsDataJoinBusinessData> list = businessDataMapper.selectASdataJoinBusinessDataForExcelDownload(params);
    ...
    public Map<String, Object> getAsDataAndBusinessDataList(SearchDto params){
        Map<String, Object> response = new HashMap<>();
        ...
        // 게시글 리스트 조회
        List<ResAsDataJoinBusinessData> list = businessDataMapper.selectASdataJoinBusinessData(params);
    ...
    /* Excel download - as */
    public void excelDataDownloadAs(List<ResAsDataJoinBusinessData> list, HttpServletResponse response) throws IOException {

        ClassPathResource classPathResource = new ClassPathResource("static/templates/excel-template-as.xlsx");
        ...
        try{
            ...
            int totCnt = list.size();
            int i = 1;
            for (ResAsDataJoinBusinessData item : list) {
                Row row = sheet.createRow(i);
                int col = 0;
                addCell(row, col++, item.getUserID().toString());
                addCell(row, col++, String.valueOf(item.getBYEAR()));
                ...
                addCell(row, col++, String.valueOf(item.getEntX()));
                addCell(row, col++, String.valueOf(item.getEntY()));
                addCell(row, col++, String.valueOf(item.getNo()));
                addCell(row, col++, String.valueOf(item.getAsDate()));
                addCell(row, col++, String.valueOf(item.getAsDescription()));
                addCell(row, col++, String.valueOf(item.getAsStatus()));
                i++;
            }
